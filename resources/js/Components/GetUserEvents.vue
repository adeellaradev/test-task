            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="relative overflow-x-auto"> 
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        <tr  v-for="(event, index) in userEvents" :key="event.event_uid" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <td class="px-6 py-4">{{ index + 1 }}</td>
                            <td class="px-6 py-4">{{ event.summary }}</td>
                            <td class="px-6 py-4">{{ new Date(event.start).toLocaleString() }}</td>
                            <td class="px-6 py-4">{{ new Date(event.end).toLocaleString() }}</td>
                            <td class="px-6 py-4"><a :href="event.meeting_url" target="_blank">Join Meeting</a></td>
                        </tr>
                        
                            <tr class="text-center pa-3">
                                <th colspan="5" class="py-7">
